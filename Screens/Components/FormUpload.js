...
import { getwithdrawalforms, submit_withdrawal_form } from '../../Backend/apiFile'
...
export default class FormUpload extends Component {
    ...
    _pickDocument = async () => {
        ...
        if (result.type = "success" && result.name) {
            await this.setState({ documentArray:result })
        }
        console.log(result);
    }
    ...
    submitDoc = async () => {
        ...
        const { documentArray } = this.state;
        if(documentArray){    
              response= await submit_withdrawal_form(documentArray.uri)
            if(response.status == "success") {
                ToastAndroid.show
                ( documentArray.name+' is uploaded successfully',
                 ToastAndroid.SHORT);
            }
            else{
                ToastAndroid.show
            ...
            ToastAndroid.show
            ('Please upload form before submitting',
             ToastAndroid.SHORT);
        }
    }   
                            ...
                            <View style={{ alignSelf: 'center', marginTop: responsiveHeight(1.5) }}>
                                ...
                                {this.state.documentArray  ? (
                                            <View>
                                                ...
                                                color="#F58B56" style={{ alignSelf: 'center' }} />
                                                <Text style={{ fontSize: 14, textAlign: 'center' }}>
                                                    {this.state.documentArray.name}</Text>
                                            </View>
                                    ...
                                    this.state.documentArrayShow.length > 0 ? (
                                        this.state.documentArrayShow.map((item, index) => {
                                            return (
                                                <TouchableOpacity
                                                    onPress={
                                                        () => Linking.openURL('https://www.pbproppants-invest.com' + item['file'])}
                                                >
                                                    <Ionicons name="document-text-outline"
                                                        size={100} color="#F58B56"
